...
func parseServerConfig(config *ss.Config) {
	...
	servers.connCnt = make([]int, len(servers.srvCipher))
...
// servers.
func createServerConn(rawaddr []byte, addr string, line chan connStat) (remote *ss.Conn, err error, srvId int) {
	...
	for i := 0; i < n; i++ {
        ...
        if srvId == -1 {
            connCnt = servers.connCnt[i]
            srvId = i
            continue
        } else {
            if connCnt > servers.connCnt[i] {
                srvId = i
                connCnt = servers.connCnt[i]
                fmt.Println("switch to id:", i, " cnt:", connCnt)
            }
            continue;
        }
	}
    ...
    if srvId > -1 {
        ...
        remote, err = connectToServer(srvId, rawaddr, addr)
		if err == nil {
			return
		}
	...
	return nil, err, srvId
}

func handleConnection(conn net.Conn, line chan connStat) {
	if debug {
	...
	remote, err, srvId := createServerConn(rawaddr, addr, line)
	if err != nil {
...
func main() {
    ...
    flag.IntVar(&core, "core", 0, "thread num")
